...
folder_dict = {
    ...
    "p8_ee_Zud_ecm91": [r'$Z^0 \to u \bar{d} \,\text{or}\, d \bar{u} \ (bkg)$', "#d1e5f0"]     
    }
...
for folder in folder_dict:
    ...
    for chunk in chunks:
        data = load_and_extract(folder, chunk)
        if data:
...
def plot_variable(varname, data, log_y=False, suffix=""):
    """
...
for varname, props in branchList.items():
    try:
        ...
        if lin_y:
            plot_variable(varname, merged, log_y=False, suffix="_linear")
        if log_y:
            plot_variable(varname, merged, log_y=True, suffix="_log")   
    except Exception as e:
        print(f"Error plotting {variables}: {e}")